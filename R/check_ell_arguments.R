...
ell_check1 <- function(fixed, pop_data, pop_domains, smp_data, smp_domains, L,transformation) {
  ...
  if (transformation=="arcsin" & min(smp_data[,fixed[[2]]])<0) {
...
ell_check2 <- function(threshold, transformation, interval, MSE, boot_type, B,
                       ...
                       benchmark_level, benchmark_weights, MSE_pop_weights, smp_subdomains=NULL, pop_subdomains=NULL) {
  ...
  if (inherits(threshold, "function") &&
    ...
    stop(strwrap(prefix = " ", initial = "",
                 ...
                 selected as threshold. See also help(ell)."))
  }
    ...
    for (i in seq_len(N_custom)) {
      ...
      if (!inherits(custom_indicator[[i]], "function")) {
        stop(strwrap(prefix = " ", initial = "",
                     ...
                     threshold. For help see Example 2 in help(ell)."))
