...
benchmark_xgb_level <- function (point_estim, framework, fixed, benchmark,
  ...
  benchmark_df <- data.frame(point_estim$ind$Mean, unique(framework$pop_data[[framework$domains]])) 
  colnames(benchmark_df) <- c("point_estim",framework$domains)
  ...
  benchmark_df <- merge(benchmark_df,crosswalk,by=framework$domains,all.x=T)
  benchmark_df <- merge(benchmark_df,benchmark_,by=superarea,all.x=T)
  ...
  if (benchmark_type == "ratio" | benchmark_type=="ratio_bound") {
      #ratio benchmarking 
        xgb_bench <- benchmark_df$point_estim*(benchmark_df$Mean/benchmark_df$weighted_pe)
  }
  if (benchmark_type=="ratio_complement") {
          factors_complement <- (1-benchmark_df$Mean)/(1-benchmark_df$weighted_pe)
          xgb_bench <- 1-(1-benchmark_df$point_estim)*factors_complement
  }
  if (benchmark_type=="ratio_bound") {
    max <- data.frame(collapse::fmax(xgb_bench,g=benchmark_df[,superarea],TRA=1))
    factors_complement <- (1-benchmark_df$Mean)/(1-benchmark_df$weighted_pe)
    xgb_bench[max>1] <- 1-(1-benchmark_df$point_estim[max>1])*factors_complement[max>1]  
  }  
   
 

  if (is.list(point_estim)) {
    point_estim_bench <- data.frame(point_estim$ind, xgb_bench)
  } else {
    point_estim_bench <- as.matrix(data.frame(point_estim, xgb_bench))
  }
  colnames(point_estim_bench)[2] <- "Mean_bench"
